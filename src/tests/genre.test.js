...
test("GET -> URL_BASE/:id, should return status code 200", async()=>{
    const res = await request(app)
...
test("PUT -> URL_BASE/genreId, should return statusCode 200, res.body.name === bodyUpdate.name", async()=>{
    const bodyUpdate ={
        name: "pop"
    }
...
test("DEL -> URL_BASE/genreId, should return statusCode 204", async()=>{
    const res = await request(app)
